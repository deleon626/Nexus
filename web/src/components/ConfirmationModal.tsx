    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-card rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {/* Schema Info */}
          <div className="mb-6">
            <div className="text-sm font-medium text-foreground mb-1">Schema</div>
            <div className="text-sm text-foreground">{data.schema_id}</div>
          </div>
              ...
              {Object.entries(editedData).map(([key, value]) => {
                ...
                return (
                  <div key={key}>
                    <label
                      htmlFor={`field-${key}`}
                      className="block text-xs font-medium text-muted-foreground uppercase mb-1"
                    >
                      {key.replace(/_/g, ' ')}
                    </label>
                    <input
                      ...
                      disabled={isSubmitting}
                      className="w-full border border-input rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent disabled:bg-muted disabled:cursor-not-allowed"
                    />
                  </div>
                );
              })}
        ...
        <div className="px-6 py-4 border-t border flex justify-end gap-3">
          <button
            onClick={onReject}
            disabled={isSubmitting}
            className="px-4 py-2 border border-input text-foreground rounded-lg hover:bg-muted disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-medium"
          >
            Reject
          </button>
          <button
            ...
            className={`px-6 py-2 rounded-lg transition-colors font-medium ${
              hasErrors
                ...
                : 'bg-primary text-primary-foreground hover:bg-primary/90 disabled:bg-primary/50 disabled:cursor-not-allowed'
            }`}
          >
